...
func _process(delta: float) -> void:
	if Input.is_action_just_pressed("zoom_up"):
		zoom = clamp(zoom + Vector2(delta,delta) * zoom_speed, Vector2(0.5,0.5), Vector2(2,2))
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.is_pressed() and event.button_index in [mouse_index_to_move, MOUSE_BUTTON_MIDDLE] and !event.double_click:
